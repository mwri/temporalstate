...
class temporalstate extends event_emitter {
    ...
    add_change (change) {
        ...
        this.emit('txn_start', {'add': {'timestamp': ts, 'name': st_name, 'val': st_val}}, txn_descr);
        txn_funs.forEach((f) => f());
        this.emit('txn_end', {'add': {'timestamp': ts, 'name': st_name, 'val': st_val}}, txn_descr);
